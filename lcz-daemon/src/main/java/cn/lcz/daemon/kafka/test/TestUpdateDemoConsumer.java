...
public class TestUpdateDemoConsumer extends AbstractUpdateConsumer {
    ...
    protected Map<UpdateTopic, List<KafkaUpdateListener>> getListeners() {
        ...
        // 添加对于每一个Topic的订阅处理
        listeners.put(UpdateTopic.TEST_UPDATE_KAFKA, getStaffRosterListeners());

        return listeners;
    }

    private List<KafkaUpdateListener> getStaffRosterListeners() {
        List<KafkaUpdateListener> listeners = Lists.newArrayList();

        listeners.add(new KafkaUpdateListener(KafkaTag.NEW) {
            @Override
        ...
        listeners.add(new KafkaUpdateListener<KafkaUpdateEntity>(KafkaTag.UPDATE) {
            @Override
        ...
        listeners.add(new KafkaUpdateListener<KafkaUpdateEntity>(KafkaTag.DELETE) {
            @Override
            protected void onUpdate(String companyId, List<KafkaUpdateEntity> oldData, List<KafkaUpdateEntity> newData) {
